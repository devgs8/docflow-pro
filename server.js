...
async function converterPdfParaWord(caminhoOrigem) {
    try {
        ...
        let conteudo = `Documento convertido de PDF para Word\n`;
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
  ...
  <Worksheet ss:Name="Análise PDF">
    <Table>
      ...
      <Row>
        <Cell><Data ss:Type="String">Ficheiro</Data></Cell>
        <Cell><Data ss:Type="String">${path.basename(caminhoOrigem).replace(/&/g,'&amp;')}</Data></Cell>
      </Row>
      <Row>
        <Cell><Data ss:Type="String">Páginas</Data></Cell>
        <Cell><Data ss:Type="Number">${nPags}</Data></Cell>
      </Row>
      <Row>
        <Cell><Data ss:Type="String">Tamanho (KB)</Data></Cell>
        <Cell><Data ss:Type="Number">${(tamanho/1024).toFixed(1)}</Data></Cell>
      </Row>
    </Table>
  </Worksheet>
</Workbook>`;
...
app.post('/api/auth/register', (req, res) => {
    try {
        ...
        logger.info(`👤 Novo utilizador:`, { email: novoUser.email });
...
app.post('/api/convert/pdf-word', upload.single('file'), async (req, res) => {
    try {
        ...
        logger.info(`✅ PDF→Word completo`, { jobId });
...
app.post('/api/convert/pdf-excel', upload.single('file'), async (req, res) => {
    try {
        ...
        logger.info(`✅ PDF→Excel completo`, { jobId });
...
app.post('/api/sign-pdf-studio', upload.single('file'), async (req, res) => {
    try {
        ...
        const rodape = `Assinado por ${signerName} em ${new Date().toLocaleString('pt-PT')} via DocFlow Pro`;
        ...
        const fonteAssinatura = await pdfDoc.embedFont(StandardFonts.Helvetica);
        const posX = pctX * width;
        const posY = pctY * height;
        pagina.drawText(`✓ ${signerName}`, {
            x: posX,
            y: posY,
            size: 14,
            font: fonteAssinatura,
            color: rgb(24, 71, 240)
        });
        ...
        fs.readdirSync(p).forEach(f => {
            try {
                ...
                if (stat.mtimeMs < limite) {
                    ...
                    logger.info(`🗑️ Ficheiro eliminado:`, { ficheiro: f });
                }
            } catch (err) {
